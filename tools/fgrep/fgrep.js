...
async function fgrep(pathname, selector) {
  const resp = await fetch(pathname);
  const text = await resp.text();
  let found = false;

  // if selector is wrapped in double quotes, search for literal string
  if (selector.slice(0, 1) === '"' && selector.slice(-1) === '"') {
    found = text.indexOf(selector.slice(1, -1)) >= 0;
  } else {
...
async function edit(path, y) {
  try {
    const statusRes = await fetch(`https://admin.hlx.page/status/hlxsites/stewart/main${path}?editUrl=auto`);
...
async function fgrepNextFile(queue, selector) {
  const path = queue.shift();
  if (path) {
    totalFiles += 1;
    fgrep(path, selector).then((result) => {
      displayResult(result);
      if (queue[0]) fgrepNextFile(queue, selector);
      endTime = new Date();
      updateStatus();
    });
  }
...
async function fgrepFiles(sitemap, selector, connections) {
  const queue = [...sitemap];
  for (let c = 0; c < connections; c += 1) {
    fgrepNextFile(queue, selector);
  }
}
...
export async function run() {
  ...
  await loadSitemap('/sitemap-en.xml');
